...
$R = \alpha S_n + \beta C + \gamma A + \delta E$
...
\bottomrule\noalign{}
\endlastfoot
$(S_n)$ & Normalized Severity Score \\
$(S)$ & Original Severity Score \\
$(S_{\min})$ & Minimum possible severity value \\
$(S_{\max})$ & Maximum possible severity value \\
\end{longtable}
...
\endlastfoot
$C$ & Asset Criticality Score \\
$D$ & Data Sensitivity Score \\
$B$ & Business Importance Score \\
$P$ & Privilege Level Score \\
$I$ & Internet Exposure Score \\
...
\bottomrule\noalign{}
\endlastfoot
$W_r$ & Overall Relationship Weight \\
$R_i$ & Security significance score of the $i$th relationship \\
$\lambda_i$ & Weight assigned to the $i$th relationship type \\
$n$ & Total number of relationship types connected to the asset \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
$K_c(v)$ & Connectivity Score of node $v$ \\
$\deg(v)$ & Number of direct relationships (degree) of node $v$ \\
$N$ & Total number of nodes in the knowledge graph \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
$P_{AP}$ & Probability of successfully exploiting the attack path \\
$p_i$ & Probability of successfully exploiting the $i$th attack step \\
$n$ & Number of attack steps in the path \\
\end{longtable}
...
\endlastfoot
$DR$ & Detection Rate (\%) \\
$N_d$ & Number of correctly detected misconfigurations \\
$N_t$ & Total number of injected misconfigurations \\
\end{longtable}
...
$OSPS = 100 \times \left(1 - \frac{\sum_{i=1}^{n} R_i}{n}\right)$
...
\bottomrule\noalign{}
\endlastfoot
$OSPS$ & Overall Security Posture Score (\%) \\
$R_i$ & Context-Aware Risk Score of the $i$th finding (normalized between 0 and 1) \\
$n$ & Total number of identified security findings \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
$C_{AI}$ & AI Recommendation Confidence Score \\
$M$ & AI Model Confidence Score \\
$E_s$ & Security Evidence Score \\
$\eta$ & Weight assigned to the AI model confidence \\
\end{longtable}
...
\subsubsection{\texorpdfstring{\textbf{AI Model Confidence (M)}}{AI Model Confidence (M)}}
...
\subsubsection{\texorpdfstring{\textbf{Security Evidence Score ($E_s$)}}{Security Evidence Score ($E_s$)}}
...
\item
$m$ = Number of security rules evaluated for each resource
\end{itemize}
...
\tightlist
\item
$f$ = Number of detected security findings
\end{itemize}
...
\tightlist
\item
$V$ = Number of nodes (cloud resources and findings)\\
\item
$E$ = Number of relationships between resources
\end{itemize}
...
\item
$E$ = Number of graph relationships
\end{itemize}
...
$R = \alpha S_n + \beta C + \gamma P_{AP} + \delta E$\\
...
\tightlist
\item
$f$ = Number of prioritized security findings.
\end{itemize}
...
$n$ = Number of cloud resources\\
\item
$m$ = Number of security rules evaluated per resource\\
\item
$V$ = Number of graph nodes\\
\item
$E$ = Number of graph relationships\\
